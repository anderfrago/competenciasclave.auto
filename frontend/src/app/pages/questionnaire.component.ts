...
@Component({ standalone: true, imports: [ReactiveFormsModule], template: `
  ...
  @if (api.questionnaire().length) {<form [formGroup]="form" (ngSubmit)="finish()">
    @for (competency of api.questionnaire(); track competency.id) {<section class="card competency-card mb-4"><div class="card-body p-4"><h2 class="h4">{{ competency.name }}</h2><p class="small-muted">Selecciona una opción para cada afirmación.</p>@for (item of competency.items || []; track item.id) {<div class="question-row"><p class="fw-medium mb-3">{{ item.statement }}</p><div class="d-flex flex-wrap gap-2">@for (option of options; track option.value) {<label class="option-label border rounded px-3 py-2" [for]="'response_' + item.id + '_' + option.value" [class.border-primary]="answerControl(item.id).value === option.value" [class.bg-primary-subtle]="answerControl(item.id).value === option.value"><input class="form-check-input me-2" type="radio" [id]="'response_' + item.id + '_' + option.value" [name]="'item_' + item.id" [formControl]="answerControl(item.id)" [value]="option.value">{{ option.label }}</label>}</div></div>}</div></section>}
